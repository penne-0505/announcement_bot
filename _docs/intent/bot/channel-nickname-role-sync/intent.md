...
2. **永続化**: Supabase Postgres（`asyncpg`）に `channel_nickname_rules` テーブルを作成し、`guild_id + channel_id` 主キーで設定を upsert する。接続文字列は `DATABASE_URL` で指定し、`app.database.Database` が接続を管理する。
...
5. **ドキュメント**: 新機能専用の plan/intent/guide/reference を追加し、README と `.env.example` へ `DATABASE_URL` を追記して Supabase ベースの起動手順を明示した。
...
- Bot 起動には Supabase Postgres の `DATABASE_URL` が必須となり、未設定時は起動前にログ出力して終了する。
