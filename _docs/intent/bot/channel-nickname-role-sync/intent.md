...
2. **永続化**: SQLite ファイル（`aiosqlite`）に `channel_nickname_rules` テーブルを作成し、`guild_id + channel_id` 主キーで設定を upsert する。ファイルパスは `DATABASE_URL` で指定し、`app.database.Database` が接続を管理する。
...
5. **ドキュメント**: 新機能専用の plan/intent/guide/reference を追加し、README と `.env.example` へ `DATABASE_URL` を追記して SQLite ベースの起動手順を明示した。

## トレードオフ
- ORM ではなく生 SQL + `aiosqlite` を選択し、依存を最小化した。複雑なマイグレーションが必要になった場合に Alembic を導入する。
...
- Bot 起動には SQLite の `DATABASE_URL` が必須となり、未設定時は起動前にログ出力して終了する。
