...
scope:
  ...
  - "登録情報を Supabase Postgres へ永続化し、`SUPABASE_URL` / `SUPABASE_KEY` を切り替えてローカル/本番を共通化する"
...
requirements:
  - "Supabase 側でテーブルが事前作成されていること"
  - "Slash コマンドはギルド内でのみ実行可能で、結果は ephemeral メッセージで通知される"
  - "チャンネル監視時は Bot が自分の編集でループしないよう配慮する"
constraints:
  - "`SUPABASE_URL` / `SUPABASE_KEY` で指定した Supabase Postgres を利用する。明示的なマイグレーションツールは導入しない"
...
migrations:
  - "Supabase の SQL Editor でテーブルを事前作成する"
rollout_plan:
  - "ローカルで Supabase 接続を指定し、`SUPABASE_URL` / `SUPABASE_KEY` で起動"
...
security_privacy:
  - "環境変数に含まれる `SUPABASE_KEY` は `.env` のみで管理しログに出力しない"
...
3. 設定は Supabase Postgres へ保存し、`SUPABASE_URL` / `SUPABASE_KEY` を切り替えるだけでローカル/他ホスト問わず永続化されるようにする。

## アーキテクチャ概要
| 層 | 役割 |
| --- | --- |
| `app.config` | `SUPABASE_URL` / `SUPABASE_KEY` を含む設定値を読み込む |
| `app.database` | Supabase SDK 接続を管理し、PostgREST 経由で CRUD を実行 |
...
## 実装ステップ
1. `pyproject` へ `supabase` を追加し、`.env.example` / README に `SUPABASE_URL` / `SUPABASE_KEY` を追記する。
2. `app.config` に Supabase 設定を追加し、`app.database` で SDK 初期化を行う。
