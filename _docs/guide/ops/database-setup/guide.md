...
3. `.env`（あるいはホストの環境変数）で `DISCORD_BOT_TOKEN` と `DATABASE_URL` を設定する。
...
4. 上記により `channel_nickname_rules`, `temporary_vc_categories`, `temporary_voice_channels` テーブルが自動作成される。必要があれば Supabase の SQL Editor で `SELECT` を実行して確認する。
