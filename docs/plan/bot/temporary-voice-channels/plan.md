...
scope:
  ...
  - "VoiceState 更新を監視して無人になったチャンネルを自動削除し、PostgreSQL 上の所有情報を同期する。"
  - "カテゴリ設定変更時に既存一時VCを破棄し、整合性の取れた状態で再作成に備える。"
non_goals:
  - "複数カテゴリや階層的なテンプレート管理。"
  - "Web ダッシュボード/外部 UI での設定編集。"
  - "複数ユーザー共同所有や権限カスタム UI。"
  - "TinyDB など別ストレージの導入（PostgreSQL を継続利用する）。"
requirements:
  functional:
  ...
  non_functional:
    ...
    - "Railway 上の Postgres と Discord API だけで完結し、追加インフラを不要にする。"
...
rollout_plan:
  ...
  - "Phase 2: Railway ステージング Bot で複数ユーザー同時操作を試験し、DB 一意制約エラーを観察。"
...
- 要件に TinyDB が挙げられているが、プロジェクトは既に PostgreSQL を前提に `Database`/`ChannelNicknameRuleRepository` を整備している。再起動後の整合性目的を満たすため、同じ技術基盤で一時VC情報も管理する。
...
| `app.database.Database` | スキーマ初期化に一時VCテーブルを追加し、既存 asyncpg プールを共有。 |
...
## 依存・前提
- discord.py 2.6.x, asyncpg 0.29.x, Python 3.12。
