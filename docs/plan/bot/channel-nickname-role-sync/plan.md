...
non_goals:
  ...
  - "PostgreSQL 以外の DB サポート"
requirements:
  - "PostgreSQL 16 系で動作し、起動時に自動でテーブルが初期化される"
...
3. 設定は PostgreSQL に保存し、Railway へデプロイ後も永続化されるようにする。
...
| リスク | 影響 | 対応 |
| --- | --- | --- |
| DB 接続失敗で Bot 起動不可 | Slash コマンド含め全停止 | 接続エラー時にログを出し再試行。Railway 側でヘルスチェックする |
