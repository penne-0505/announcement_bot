...
## 目的
- Discord 上でニックネーム打刻チャンネルを自動化し、投稿者のニックネームを本文へ強制同期しつつロールを自動付与する機能の確定仕様を記録する。
...
| `src/bot/handlers.py` | 監視対象チャンネルの投稿をニックネームへ上書きし、ロールを付与する。 |
...
3. `enforce_nickname_and_role`:
   - 投稿者の `display_name` → `global_name` → `name` の順で表示名を決定。
   - 現在の本文と異なれば `message.edit(content=display_name, allowed_mentions=AllowedMentions.none())`。
